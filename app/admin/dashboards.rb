ActiveAdmin::Dashboards.build do
  ...
  section "New Teams" do
    table_for Team.order("created_at DESC").limit(5) do
        column :name do |team|
          link_to team.name, [:admin, team]
        end
      end
    strong { link_to "View All Teams", admin_teams_path}
  end

  section "New Players" do
    table_for Player.order("created_at DESC").limit(5) do
        column :name do |player|
          link_to player.name, [:admin, player]
        end
        column :is_foreign_player
      end
